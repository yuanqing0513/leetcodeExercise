...
public class Solution8 implements Solution{
    ...
    public int myAtoi(String str) {
        ...
        for (; startPos < strArr.length; startPos++) {
            if (strArr[startPos] >= '0' && strArr[startPos] <= '9') {
                sb.append(strArr[startPos]);
            }else {
                break;
            }
        }

        String numStr = sb.toString().replaceAll("^[0]+", "");
        if (numStr.length() == 0) {
            return 0;
        }
        String base = "";
        ...
        if (numStr.length() > base.length()) {
            return Integer.parseInt(base);
        }else if (numStr.length() == base.length()){
            if (numStr.compareTo(base) > 0) {
                return Integer.parseInt(base);
            }else {
